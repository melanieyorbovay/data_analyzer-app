...
class DataAnalyzer:
    ...
    def refresh_preview(self):
        ...
        # Reconfigure les colonnes avec les vrais noms du fichier
        cols = list(self.df.columns[:4])
        ...
        for i in range(min(10, len(self.df))):
            row = [str(x)[:10] for x in self.df.iloc[i, :4].values]
...
"""
        ...
        if len(numeric_cols) > 0:
            ...
            for col in numeric_cols:
                ...
                if len(serie) > 0:
                    ...
                    stats += f" Médiane: {serie.median():.1f}\n"
    ...
    def plot_graph(self, type_graph):
        ...
        if type_graph == 'hist' and len(numeric.columns) > 0:
            ...
            ax.set_title(f"Histogramme : {numeric.columns[0]}")
        ...
        plt.close(fig)  # Sinon fuite de mémoire
